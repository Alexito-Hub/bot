const { proto, jidDecode, downloadContentFromMessage } = require('@whiskeysockets/baileys')
...
const client = (sock) => {
    ...
    sock.getBuffer = async (mediakey, MediaType) => {
        try {
            const stream = await downloadContentFromMessage(mediakey, MediaType);
            if (!stream) {
                console.error('Error al obtener el stream:', stream);
                throw new Error('No se pudo obtener el stream de medios.');
            }
    ...
    sock.downloadMediaMessage = async (message) => {
        try {
            ...
            const buffer = await sock.getBuffer(message, messageType);

            return buffer;
        } catch (error) {
            console.error('Error al descargar el contenido del mensaje multimedia:', error);
            throw error;
        }
    }
	sock.isFromAdmin = async (m) => {
        try {
            const admins = await sock.getGroupAdmins(m.chat);
            return admins.includes(m.sender);
        } catch (error) {
            console.error('Error al verificar si el remitente es un administrador:', error);
            return false;
        }
    };
	return sock
...
const sms = async(sock, m) => {
	...
	m.replyMention = (teks = '', options = {}) => {
	    const mentions = options.mentions || sock.parseMention(teks);
	    sock.sendMessage(options.id || m.chat, { text: teks, mentions }, { quoted: options.quoted || m });
	};
