...
module.exports = {
    ...
    async execute(sock, m, args) {
        try {
            ...
            try {
                await sock.sendMessage(m.chat, {text: "cargando"}, {quoted: m}); 
                const res = await TwitterDL(twiUrl);
                if (res?.result.type == 'video') {
                    ...
                    for (let i = 0; i < res.result.media.length; i++) {
                        await sock.sendMessage(m.chat, {video: {url: res.result.media[i].result[0].url}, caption: caption}, {quoted: m});
                    ...
                    const caption = res?.result.caption ? res.result.caption : '*Aquí tiene su imagen*';
                    for (let i = 0; i < res.result.media.length; i++) {
                        await sock.sendMessage(m.chat, {image: {url: res.result.media[i].url}, caption: caption}, {quoted: m});
