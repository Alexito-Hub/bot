...
module.exports = async(sock, m, store) => {
	try {
        ...
        const commandBody = hasCommandPrefix && m.body ? m.body.slice(prefixes.find(prefix => m.body.toLowerCase().startsWith(prefix.toLowerCase())).length).trim() : m.body ? m.body.trim() : '';
